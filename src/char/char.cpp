...
int32 char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count ) {
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 58, SQLDT_UINT8, &p.show_costumes, 0, nullptr, nullptr)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
int32 char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 76, SQLDT_UINT8, &p->show_costumes, 0, nullptr, nullptr)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
