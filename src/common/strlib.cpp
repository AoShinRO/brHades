...
bool sv_readdb( const char* directory, const char* filename, char delim, size_t mincols, size_t maxcols, size_t maxrows, bool (*parseproc)( char* fields[], size_t columns, size_t current ), bool silent ){
	...
	while( fgets( line, static_cast<int>( maxcols * colsize ), fp ) ){
		...
		if( columns < mincols )
		{
			ShowError("sv_readdb: Insufficient columns in line %d of \"%s\" (found %zu, need at least %lu).\n", lines, path, columns, mincols);
			continue; // not enough columns
		}
		if( columns > maxcols )
		{
			ShowError("sv_readdb: Too many columns in line %d of \"%s\" (found %zu, maximum is %lu).\n", lines, path, columns, maxcols );
	...
	aFree(line);
	fclose(fp);
	ShowStatus("Concluida a leitura das entradas '" CL_WHITE "%zu" CL_RESET "' em '" CL_WHITE "%s" CL_RESET "'.\n", entries, path);
