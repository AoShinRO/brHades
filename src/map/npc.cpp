...
void npc_loadsrcfiles() {
	...
	ShowInfo ("Carregamento concluído '" CL_WHITE "%d" CL_RESET "' NPCs:" CL_CLL "\n"
		...
		"\t-'" CL_WHITE "%d" CL_RESET "' Mobs não armazenados em cache\n",
...
static void npc_parsename(struct npc_data* nd, const char* name, const char* start, const char* buffer, const char* filepath)
{
	...
	if( p ) { // <Display name>::<Unique name>
		...
		if( len > NPC_NAME_LENGTH ) {
			ShowWarning("npc_parsename: O nome de exibição de '%s' é muito longo (len=%u) no arquivo '%s', linha '%d'. Truncando para %u caracteres.\n", name, (unsigned int)len, filepath, strline(buffer,start-buffer), NPC_NAME_LENGTH);
		...
		len = strlen(p+2);
		if( len > NPC_NAME_LENGTH )
			ShowWarning("npc_parsename: O nome exclusivo de '%s' é muito longo (len=%u) no arquivo '%s', linha '%d'. Truncando para %u caracteres.\n", name, (unsigned int)len, filepath, strline(buffer,start-buffer), NPC_NAME_LENGTH);
		safestrncpy(nd->exname, p+2, sizeof(nd->exname));
	} else {// <Display name>
		size_t len = strlen(name);
		if( len > NPC_NAME_LENGTH )
			ShowWarning("npc_parsename: O nome '%s' é muito longo (len=%u) no arquivo '%s', linha '%d'. Truncando para %u caracteres.\n", name, (unsigned int)len, filepath, strline(buffer,start-buffer), NPC_NAME_LENGTH);
		safestrncpy(nd->name, name, sizeof(nd->name));
	...
	if( *nd->exname == '\0' || strstr(nd->exname,"::") != nullptr ) {// invalid
		...
		ShowWarning("npc_parsename: Nome exclusivo inválido no arquivo '%s', linha '%d'. Renomeando '%s' para '%s'.\n", filepath, strline(buffer,start-buffer), nd->exname, newname);
	...
	while ( p ) {
		...
		if (value == 0 && (type == NPCTYPE_SHOP || type == NPCTYPE_MARKETSHOP)) { // NPC selling items for free!
			ShowWarning("npc_parse_shop: Item %s [%u] is being sold for FREE in file '%.*s', line '%d'.\n",
				id->name.c_str(), nameid2, (int)filepath.size(), filepath.data(), line);
		}
		if( ( type == NPCTYPE_SHOP || type == NPCTYPE_MARKETSHOP ) && value*0.75 < id->value_sell*1.24 ) { // Exploit possible: you can buy and sell back with profit
			ShowWarning("npc_parse_shop: Item %s [%u] discounted buying price (%d->%d) is less than overcharged selling price (%d->%d) at file '%.*s', line '%d'.\n",
				id->name.c_str(), nameid2, value, (int)(value*0.75), id->value_sell, (int)(id->value_sell*1.24), (int)filepath.size(), filepath.data(), line);
		}
		if (type == NPCTYPE_MARKETSHOP && qty < -1) {
			ShowWarning("npc_parse_shop: Item %s [%u] is stocked with invalid value %hd, changed to unlimited (-1). file '%.*s', line '%d'.\n",
				id->name.c_str(), nameid2, qty, (int)filepath.size(), filepath.data(), line);
			qty = -1;
		}
...
static void npc_market_fromsql(void) {
	...
	while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
		...
		if (id == nullptr) {
			ShowWarning("npc_market_fromsql: Item de venda inválido na tabela '%s' (id '%u').\n", market_table, list.nameid);
			continue;
		}

		if (list.value * 0.75 < id->value_sell * 1.24) { // Exploit possible: you can buy and sell back with profit
			ShowWarning("npc_market_fromsql: O preço de compra com desconto (%d->%d) do item %s [%u] é menor que o preço de venda cobrado a mais (%d->%d) na tabela '%s'. Atribuindo ao valor de venda atual.\n",
	...
	ShowStatus("Carregamento concluído de '" CL_WHITE "%d" CL_RESET "' entradas para '" CL_WHITE "%d" CL_RESET "' Mercados de NPC da tabela '" CL_WHITE "%s" CL_RESET "'.\n",count, db_size(NPCMarketDB), market_table);
}
#endif
