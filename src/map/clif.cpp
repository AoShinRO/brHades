	...
	case CHAT_WOS:
		{
			...
			if (sd) {
				...
				cd = reinterpret_cast<chat_data*>(bl);
			} else break;
			if (cd == nullptr)
				break;
...
void clif_dispchat(chat_data& cd, map_session_data* sd) {

	if (cd.usersd[0] == nullptr || cd.owner == nullptr)
		return;
	...
	if (cd.owner->type == BL_NPC) {
		...
		if (cd.limit)
			p->flag = FLAG_CHAT_ARENA;
		else
			p->flag = FLAG_CHAT_PK;
	}
	else if (cd.owner->type == BL_PC && cd.pub == false) {
		p->flag = FLAG_CHAT_PRIVATE;
	}
	else {
		p->flag = FLAG_CHAT_PUBLIC;
	}
...
///     0 = left
///     1 = kicked
void clif_chat_leave(chat_data& cd, map_session_data& sd, bool flag){

	PACKET_ZC_MEMBER_EXIT p{};

	p.packetType = HEADER_ZC_MEMBER_EXIT;
	p.playersRemaining = cd.users - 1;
	safestrncpy(p.exitPlayername,sd.status.name,sizeof( p.exitPlayername ));
	p.flag = flag;
...
static void clif_getareachar_pc(map_session_data* sd,map_session_data* dstsd)
{
	...
	if( dstsd->chatID ) {
		...
		if( (cd = map_id2cd(dstsd->chatID)) && cd->usersd[0]==dstsd)
			clif_dispchat(*cd,sd);
	...
	case BL_NPC:
		{
			TBL_NPC* nd = (TBL_NPC*)bl;
			if( nd->chat_id )
				clif_dispchat(*map_id2cd(nd->chat_id),sd);
			if( nd->size == SZ_BIG )
				clif_specialeffect_single(bl,EF_GIANTBODY2,sd->fd);
			else if( nd->size == SZ_MEDIUM )
				clif_specialeffect_single(bl,EF_BABYBODY2,sd->fd);
...
void clif_changemapcell(int16 m, int32 x, int32 y, int32 type, enum send_target target, block_list* bl){
...
/// 09ca <lenght>.W <id> L <creator id>.L <x>.W <y>.W <unit id>.L <range>.B <visible>.B <skill level>.B (ZC_SKILL_ENTRY5)
void clif_skill_unit_test(block_list &bl, uint16 x, uint16 y, int32 unit_id, uint8 range, uint8 skill_lv) {
...
void clif_skillup(map_session_data &sd, uint16 skill_id, int32 lv, int32 range, bool upgradable) {
...
void clif_skill_scale( struct block_list *bl, int32 src_id, int16 x, int16 y, uint16 skill_id, uint16 skill_lv, int casttime ){
...
int32 clif_skill_damage(struct block_list *src,struct block_list *dst,t_tick tick,int32 sdelay,int32 ddelay,int64 sdamage,int32 div,uint32 skill_id,uint16 skill_lv,enum e_damage_type type)
{
 ...
 * @param val3
 */
void clif_status_change(struct block_list *bl, e_efst_type type, bool flag, t_tick tick, int32 val1, int32 val2, int32 val3) {
	map_session_data *sd = nullptr;
...
void clif_parse_Emotion(int fd, map_session_data *sd){
	...
	if (battle_config.basic_skill_check == 0 || pc_checkskill(sd, NV_BASIC) >= 2 || pc_checkskill(sd, SU_BASIC_SKILL) >= 1) {
		...
		if(battle_config.client_reshuffle_dice && emoticon>=ET_DICE1 && emoticon<=ET_DICE6) {// re-roll dice
			emoticon = util::clamp<decltype(emoticon)>( rnd()%6+ET_DICE1 );
		}
...
static void clif_parse_GuildAllianceMessage(int fd, map_session_data* sd){
	...
	clif_guild_alliance_message(g->guild, output, util::clamp<uint32>(strlen(output)));
}
