...
void searchstore_click(map_session_data& sd, uint32 account_id, int store_id, t_itemid nameid)
{
	...
	ARR_FIND(0, util::clamp<uint32>(sd.searchstore.items.size()), i, sd.searchstore.items[i]->store_id == store_id && sd.searchstore.items[i]->account_id == account_id && sd.searchstore.items[i]->nameid == nameid);
