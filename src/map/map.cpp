	...
	if( enable_grf )
		...
		for(const auto &mapdat : mapcachefilepath) {
			...
			if( ( fp = fopen(mapdat.c_str(), "rb")) == nullptr) {
				ShowFatalError( "Nao foi possível abrir o arquivo de cache do mapa " CL_WHITE "%s" CL_RESET "\n", mapdat.c_str());
				continue;
			}
...
int log_sql_init(void)
{
	...
	if ( SQL_ERROR == Sql_Connect(logmysql_handle, log_db_id.c_str(), log_db_pw.c_str(), log_db_ip.c_str(), log_db_port, log_db_db.c_str()) ){
		ShowError("Nao foi possível conectar com uname='%s',host='%s',porta='%hu',banco de dados='%s'\n",
