...
bool itemdb_parse_roulette_db(void)
{
	...
	for (i = 0; i < MAX_ROULETTE_LEVEL; i++) {
		...
		if (rd.items[i] > limit) {
			ShowWarning("itemdb_parse_roulette_db: nível %d tem %d itens, apenas %d suportados, limitando...\n", i + 1, rd.items[i], limit);
			rd.items[i] = limit;
			continue;
		}

		/** this scenario = rd.items[i] < limit **/
		ShowWarning("itemdb_parse_roulette_db: O nível %d tem %d itens, %d sao obrigatórios. Preenchendo com Macas...\n", i + 1, rd.items[i], limit);
