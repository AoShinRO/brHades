...
int web_sql_init(void) {
	...
	if (SQL_ERROR == Sql_Connect(login_handle, login_server_id.c_str(), login_server_pw.c_str(), login_server_ip.c_str(), login_server_port, login_server_db.c_str())) {
		ShowError("Não foi possível conectar com uname='%s',host='%s',porta='%hu',banco de dados='%s'\n",
	...
	ShowStatus("Conexão bem-sucedida! (Conexão do servidor de login)\n");
	...
	ShowStatus("Conexão bem-sucedida! (Conexão do servidor Char)\n");
	...
	ShowStatus("Conexão bem-sucedida! (Conexão do servidor de mapas)\n");
	...
	ShowStatus("Conexão bem-sucedida! (Conexão com o servidor Web)\n");
...
{
	ShowStatus("Fechar conexão de login com o banco de dados...\n");
	Sql_Free(login_handle);
	login_handle = nullptr;
	ShowStatus("Fechar conexão de banco de dados Char...\n");
	Sql_Free(char_handle);
	char_handle = nullptr;
	ShowStatus("Fechar conexão de banco de dados Map...\n");
	Sql_Free(map_handle);
	map_handle = nullptr;
	ShowStatus("Fechar conexão de banco de dados Web...\n");
	Sql_Free(web_handle);
	web_handle = nullptr;
...
bool WebServer::initialize( int argc, char* argv[] ){
	...
	for (int i = 0; i < 10; i++) {
		...
		if (http_server->is_running())
			break;
		ShowDebug("Servidor web não está em execução, hibernando por 1 segundo.\n");
		std::this_thread::sleep_for(std::chrono::seconds(1));
	}

	if (!http_server->is_running()) {
		ShowError("O servidor web não iniciou, parando.\n");
		return false;
	}

	ShowStatus("O servidor web está " CL_GREEN "pronto" CL_RESET " (o servidor está escutando na porta %u).\n\n", web_config.web_port);
	return true;
#endif
}
